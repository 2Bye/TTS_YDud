...
def get_symbols(symbol_set='english_basic'):
    if symbol_set == 'english_basic_withoit_tokens':
        _pad = '_'
        _punctuation = '!\'(#),.:;? '
        _special = '-'
        ...
        _special = '-'
        _letters = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz'
        _special_tokens = ['@pause', '@happy', '@sad', '@neutral',
                           '@angry', '@disgust', '@fear', '@enthusiastic']
        symbols = list(_pad + _special + _punctuation + _letters) + _arpabet + _special_tokens
...
def get_pad_idx(symbol_set='english_basic'):
    if symbol_set in {'english_basic', 'english_basic_lowercase', 'russian_basic', 'english_basic_withoit_tokens'}:
        return 0
    else:
